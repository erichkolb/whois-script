...
else if(isset($_POST['submit'])) 
{
	...
	$permalink = xssClean(mres($_POST['permalink']));
	...
	$array = mysql_fetch_array(mysqlQuery("SELECT id FROM page_language WHERE id='$id' AND language ='$language';"));
	if(isset($array['id']))
	...
	if (!$error)
	{
		add_page_language($id, $permalink, $title,$content,$language);
	}
...
?>
	<title>ADD Page Language: <?php echo(get_title());?></title>
	</head>
	<body>
	...
	<div id="wrapper">
			<div id="page-wrapper">
				<div class="row page-ttl">
					<div class="col-lg-12">
						<h1>
							<i class="fa fa-sitemap"></i> Add <?php echo ucfirst($permalink); ?> Page language <small> Add new language for <?php echo ucfirst($permalink); ?> page</small>
						</h1>
					</div>
				</div><!-- /.row -->
				<div class="page-content">
					...
					$sql = mysqlQuery("SELECT `language` FROM page_language WHERE id = '$id'");
                    ...
                    {					
					?>
					<form role="form" action="add_page_language.php" method="post">
					    <?php 
						if(isset($_POST['id']))
						{
						?>
						...
						{
						?>
						<input type="hidden" name="permalink" value="<?php echo $_POST['permalink']; ?>" >
						<?php 
						}
						else
						{
						...
						<div class="form-group">
							<a href="page_language.php?id=<?php echo $id ; ?>"><button type="button" class="btn btn-default"><i class="fa fa-chevron-left"></i> Back </button></a>                           							
					...
					<div class="panel panel-success">
						<div class="panel-body page-list-setting">
							<div class='col-md-12 content text-center'>				
								<h1>All Language Exist For this Page !</h1>
							</div>
						</div>	
					</div>	
