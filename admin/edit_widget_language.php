...
if (isset($_GET['id']) && is_numeric($_GET['id']) && isset($_GET['language'])) 
{
	...
	$array = mysql_fetch_array(mysqlQuery("SELECT * FROM `widget_language` WHERE `id`='$id' AND language = '$language'"));
...
else if(isset($_POST['submit'])) 
{
	...
	$title = xssClean(mres($_POST['title']));
	...
	if(!$error)
		edit_widget_language($id,$content,$language,$title);
} 
else 
{
...
if(isset($id))
	$_SESSION['edit_id'] = $id;
?>
	<title>Edit Widget Language: <?php echo(get_title());?></title>
	</head>
	<body>
	...
	<div id="wrapper">
		<div id="page-wrapper">
			<div class="row page-ttl">
				<div class="col-lg-12">
					<h1>
						<i class="fa fa-wordpress"></i> <?php echo ucfirst($title); ?> Language <small>Edit <?php echo ucfirst($_SESSION['edit_language']) ; ?> Language of <?php echo ucfirst($title); ?> widget</small>
					</h1>
				</div>
			</div>
				...
				<div class="margin_sides">
					<div class="row">
						<div class="col-lg-8 col-md-10 col-sm-12 col-xs-12">
							...
							?>
							<form role="form" action="edit_widget_language.php" method="post">
								...
								<div class="form-group">
									<a href="widget_language.php?id=<?php echo $_SESSION['edit_id']; ?>"><button type="button" class="btn btn-default"><i class="fa fa-chevron-left"></i> Back </button></a> <button name="submit" type="submit" class="btn btn-success"><i class="fa fa-check"></i> Update</button>
								</div>
							</form>
						</div>
					</div>
